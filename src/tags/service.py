...
class TagService:
    ...
    async def add_tag(self, tag_data: TagCreateModel, session: AsyncSession):
        normalized = tag_data.name.strip().lower().capitalize()

        tag = await self.get_tag_by_name(normalized, session)
    ...
    async def update_tag(
        ...
        normalized = tag_update_data.name.strip().lower().capitalize()
        tag_exist = await self.get_tag_by_name(normalized, session)
